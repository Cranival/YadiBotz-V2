...
exports.isLimit = function(sender, isPremium, isOwner, limitCount, _db){
    ...
    if (found === false) {
        ...
        _db.push(obj)
        fs.writeFileSync('./database/limit.json', JSON.stringify(_db, null, 3))
        return false
    }
}
...
exports.addBalance = function(sender, duit, _db){
    ...
    if (position !== false) {
        ...
        fs.writeFileSync('./database/balance.json', JSON.stringify(_db, null, 3))
    } else {
        const bulin = ({
            id: sender,
            balance: duit
...
exports.isGame = function(sender, isOwner, gcount, _db){
    ...
    if (found === false){
        ...
        _db.push(obj);
        fs.writeFileSync('./database/glimit.json',JSON.stringify(_db, null, 3));
        return false;
    }
}
